...
const EditBlog=()=>{
  ...
  useEffect(()=>{
    async function fetchData() {
        ...
        if(foundBlog){
            setTopic(foundBlog.title)
            setBlogContent(foundBlog.content)
            setAuthorName(foundBlog.authorName)
            setImageURL(foundBlog.imageURL);
        }
    }
    fetchData();
  },[])
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.put('https://blogs-ooi1.onrender.com/api/v1/blogs', {
        topic,
        content: blogContent,
